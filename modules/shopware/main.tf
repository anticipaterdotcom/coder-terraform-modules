...
resource "coder_agent" "shopware" {
  ...
  startup_script = <<-EOT
    ...
    curl -fsSL https://code-server.dev/install.sh | sh -s -- --method=standalone --prefix=/tmp/code-server --version 4.99.4
    ...
    mysql -u root -proot -e "USE shopware; UPDATE sales_channel_domain SET url = REPLACE(url, 'http://', 'https://') WHERE url LIKE 'http://%';"
